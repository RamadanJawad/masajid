...
class FlutterQuran {
  ...
  List<String> getAllJozzs() => QuranConstants.quranJuz
      .map((jozz) => "juz $jozz")
      .toList();

  ///[getAllHizbs] returns list of all Quran hizbs' names
  List<String> getAllHizbs() =>
      QuranConstants.quranHizbs.map((jozz) => "Hizb $jozz").toList();
  ...
  List<Bookmark> getUsedBookmarks() =>
      AppBloc.bookmarksCubit.defaultBookmarks.where((b) => b.page != -1).toList();
